...
var cmdRun = &Command{
	UsageLine: "run [appname] [watchall] [-main=*.go] [-downdoc=true]  [-gendoc=true] [-vendor=true] [-e=folderToExclude]  [-tags=goBuildTags] [-runmode=EGO_RUNMODE]",
...
func init() {
	...
	cmdRun.Flag.StringVar(&runmode, "runmode", "", "Set the ego run mode.")
	exit = make(chan bool)
}
...
func runApp(cmd *Command, args []string) int {
	if len(args) == 0 || args[0] == "watchall" {
		...
		if found, _gopath, _ := SearchGOPATHs(currpath); found {
			...
			// appname = path.Base(currpath)
			// currentGoPath = currpath
		}
	} else {
		// Check if passed ego application path/name exists in the GOPATH(s)
	...
	if runmode == "prod" || runmode == "dev" {
		os.Setenv("EGO_RUNMODE", runmode)
		logger.Infof("Using '%s' as 'runmode'", os.Getenv("EGO_RUNMODE"))
	} else if runmode != "" {
		os.Setenv("EGO_RUNMODE", runmode)
		logger.Warnf("Using '%s' as 'runmode'", os.Getenv("EGO_RUNMODE"))
	} else if os.Getenv("EGO_RUNMODE") != "" {
		logger.Warnf("Using '%s' as 'runmode'", os.Getenv("EGO_RUNMODE"))
	}
